...
public class SpecificationMapper {
    ...
    /**
     * Maps the given Skillfile sf on the new Specification tc. The method creates a new Skillfile that is created at
     * targetPath. The parameter mappigfile is a string with a path to a mapping file that contains name mappings for
     * types and fields.
     * 
     * @param tc
     *            - TypeContext of the new specification
     * @param sf
     *            - Skillfile to map
     * @param targetPath
     *            - Path where the new Skillfile should be created
     * @param mappingfile
     *            - Mappingfile with name to name mappings
     * @return - new SKillfile with typesystem as in the TypeContext and information from Skillfile sf
     * @throws ParseException
     *             - thrown if the mapping file given by the string mappingfile is syntactically wrong
     * @throws FileNotFoundException
     *             - thrown if string mappingfile is not the path to a mapping file
     * @throws IOException
     *             - thrown if the new Skillfile could not be created
     * @throws InterruptedException
     *             - thrown if the object allocation in the new Skillfile fails
     * @throws SkillException
     *             - thrown if some restriction check fails
     */
    ...
    /**
     * Maps the old typesystem to the new typesystem. This is a type to type mapping. Types are mapped by name
     * equivalence (mapping file can change names). If no equivalent name can be found, the method tries to map the type
     * to one of his supertypes. If no mapping can be found at all, the type is mapped to null.
     */
    private void mapStates() {
        ...
        for (StoragePool<?, ?> base : oldState.getTypes()) {
            if (base instanceof BasePool<?>) {
                int lbpo = 0;

                for (StoragePool<?, ?> oldPool : new TypeHierarchyIterator<>(base)) {
                    ...
                    if (newPool == null) {
                        ...
                        if (newPool != null) {
                            // if we can project the old pool, we need the offset map later to refer to the types
                            // instances
    ...
    /**
     * Transfer the objects from old state to new state. This is done by transferring the object information and then
     * allocating the objects in new typesystem.
     * 
     * @throws InterruptedException
     *             - thrown if there happens an error while allocating objects
     */
    ...
    private <T> void transferObjects() throws InterruptedException {
        ...
        for (StoragePool<?, ?> base : newState.getTypes()) {
            if (base instanceof BasePool<?>) {
                int lbpo = 0;

                for (StoragePool<?, ?> newPool : new TypeHierarchyIterator<>(base)) {
    ...
    /**
     * Search field with name fieldname in the fields of type new pool. The fields inherited by supertypes are included
     * in the search.
     * 
     * @param fieldname
    ...
    private FieldDeclaration<?, ?> searchField(String fieldname, StoragePool<?, ?> newPool) {
        ...
        while (newFieldIt.hasNext()) {
            ...
            if (newField.name().equals(fieldname))
                return newField;
        }
    ...
    /**
     * Transfer the field data. The compatibility of fields is checked before this step. The data is transferred for
     * every type that has the field independently. This is because a field in the old typesystem could be splitted in
     * multiple field in the new typsystem.
     * 
     * @param oldField
     ...
     *            - data to this field
     * @param oldPool
     *            - field data for the objects of this type
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Return the typename of the type in the new typesystem. This could be influenced by mapping file.
     * 
     * @param name
     *            - name of type in old typesystem
    ...
    /**
     * Return the fieldname of the field in the new typesystem. This could be influenced by mapping file.
     * 
     * @param oldField
     *            - field in old typesystem
    ...
    /**
     * Calculate the corresponding Skillobject in the new typesystem. Call this method only for Skillobjects that are
     * present in the new typesystem, otherwise a nullpointer exception is thrown.
     * 
     * @param oldObject
     *            - the object for which we want to calculate the corresponding SKillobject
    ...
    protected SkillObject calculateNewSkillObject(SkillObject oldObject) {
        ...
        int id = oldObject.getSkillID() - oldLbpoMap[oldPool.typeID - 32] + newLbpoMap[newPool.typeID - 32]
                + projectionOffsetMap[oldPool.typeID - 32];

        SkillObject r = newPool.getByID(id);
        assert null != r;
        return r;
    }
    ...
    private ValueConversion dispatchConversion(FieldType<?> newType) {
        ...
        if (id == 6 || id == 14)
            return StandardConversion.get();
        else if (7 <= id && id <= 13)
            return NumberConversion.get(id);
        else if (15 <= id && id <= 19)
            return new LinearCollectionConversion(newType);
        else if (id == 20)
            return new MapConversion(newType);
        else
            return usertypeConv;
    }
    ...
    private class LinearCollectionConversion extends ValueConversion {
        ...
        private LinearCollectionConversion(FieldType<?> newType) {
            int id = newType.typeID;
            if (id == 15 || id == 17)
                this.collectionCreator = arrayCreator;
            else if (id == 18)
                this.collectionCreator = listCreator;
            else
                this.collectionCreator = setCreator;
    ...
    private class MapConversion extends ValueConversion {
        ...
        Object convert(Object o) {
            ...
            for (Entry<?, ?> oldEntry : ((Map<?, ?>) o).entrySet()) {
                newMap.put(keyTypeConversion.convert(oldEntry.getKey()),
                        valueTypeConversion.convert(oldEntry.getValue()));
            }
